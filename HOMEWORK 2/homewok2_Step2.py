...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 12:15:32 2017
...
#        cset = ax.contour(XX, YY, ZZ, zdir='z', offset=-0.45, cmap=pl.cm.coolwarm)
...
#    pl.yscale('log')
    ...
    unorm = pl.plot(range(len(norm_disp)),norm_disp,'-m^',label = 'Norm')
    pl.yscale('log')
    pl.grid()
...
#%%
    LevelsClassification = [0.012, 0.013]
    pl.figure()
    ...
    pl.axis(axesList)
    pl.grid()
    pl.savefig('LevelsClassification.eps')
    pl.show()
    ...
    CS = pl.contour(XX, YY, ZZg1, [0.006], cmap = mapaColores)#Red
    pl.clabel(CS, inline=1, fontsize=10)
    CS = pl.contour(XX, YY, ZZg2, [0.0065], cmap = mapaColores)#Blue
    pl.clabel(CS, inline=1, fontsize=10)
    CS = pl.contour(XX, YY, ZZg3, [0.0065], cmap = mapaColores)#Green
    pl.clabel(CS, inline=1, fontsize=10)
    pl.axis(axesList)
    pl.grid()
    pl.savefig('LevelsClassificationInsight.eps')