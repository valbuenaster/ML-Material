...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 20 15:02:07 2017
...
def calculate_distortionMeasure(List_mu,List_CVMatrices,r_k,X,Features):
    summation = 0
    
    for jj in range(Features):
        for ii in range(len(X)):
            difference = List_mu[jj] - X[ii]
            summation = summation + r_k[jj][ii]*np.dot(difference.T,difference)
    return summation
...
def updateParameters_k_means(vector_pi, X, muM,CVM, Features):
    r_k = []
    New_muM = []
    New_CVM = []
    ...
    #E-step
#    for kk in range(Features):
#        r_k.append(calculate_r_ik_K_MEANS(vector_pi,X,muM,CVM,kk))
    r_k = calculate_r_ik_K_MEANS(X,muM,CVM,Features)  
    r_k_columns = []    
    for kk in range(Features):
        aux_r_k_kk = []
        for element in r_k:
            aux_r_k_kk.append(element[kk]) 
            
        dupleTemp = calculate_pk_rk(aux_r_k_kk)
        rk = dupleTemp[0]     
        New_muM.append(calculate_mu(rk,aux_r_k_kk,X))
        New_CVM.append(calculate_CVM(rk,aux_r_k_kk,X,New_muM[-1]))
        
        r_k_columns.append(aux_r_k_kk)
        
    return (New_muM,New_CVM,r_k_columns)
...
def computeSurfaceZ_ind_dist(XX,YY,mu,CV):
    dimX = len(XX)
    dimY = len(XX[0])
    Z = np.zeros((dimX,dimY)) 
    for ii in range(dimX):
        for jj in range(dimY):
            vect_x = np.matrix([[XX[ii][jj]],[YY[ii][jj]]])
            Z[ii][jj] = calculate_probability_MultiGaussian(vect_x,mu, np.matrix(CVm))
    return Z   
...
#    print inverseCV
    for x in X:
        difference = mu - x
        if(difference.T*inverseCV*difference<0):
            print "mu"
            print mu
            print "CV"
            print CV
        arrayDistances.append(math.sqrt(difference.T*inverseCV*difference))
#        arrayDistances.append(difference.T*inverseCV*difference)
...
def calculate_r_ik_K_MEANS(X,mu,CV,Features):
    ...
    for x in X:
        ...
        for ii in range(Features):
            ...
            if(distances[-1]<0.0):
                print "Negative "+str(distances[-1])
...
#    max_y = 10
    ...
    ListRealizationsValuesLLGMMTrain = []
    ListRealizationsValuesLLGMMTest = []   
    
    for mm in range(Realizations):
...
#        XTrain = D1Train + D2Train + D3Train
        ...
        MaximumLikelihood = -9999999999
    #%% 
        iteration = 1
    #    while(True):   
        while(iteration < NIterations):
            stringName = "\nRealization: "+str(mm)+", Iteration: " + str(iteration)
            ...
            ListRealizationsValuesLLGMMTrain.append(ValuesLLGMMTrain)
            ListRealizationsValuesLLGMMTest.append(ValuesLLGMMTest)
            print "Log likelihood "+ str(ValuesLLGMMTest[-1])
            
            if(MaximumLikelihood<ValuesLLGMMTest[-1]):
                MaximumLikelihood = ValuesLLGMMTest[-1]
                maximumLista_mu = list(Lista_mu)
                maximumLista_CV = list(Lista_CV)
                maximumv_pi = list(v_pi)
            ...
            print "norm(tempM) "+str(NormMagnitudeDisplacement)
...
#    ListRealizationsValuesLLGMMTrain = []
#    ListRealizationsValuesLLGMMTest = [] 
    avr_RealizationsValuesLLGMMTrain = []
    avr_RealizationsValuesLLGMMTest = []
    
    for ff in range(len(ListRealizationsValuesLLGMMTrain[0])):
        valueA = 0
        valueB = 0
        for gg in range(len(ListRealizationsValuesLLGMMTrain)):
            valueA = valueA + ListRealizationsValuesLLGMMTrain[gg][ff]
            valueB = valueB + ListRealizationsValuesLLGMMTest[gg][ff]
        avr_RealizationsValuesLLGMMTrain.append(valueA/len(ListRealizationsValuesLLGMMTrain))
        avr_RealizationsValuesLLGMMTest.append(valueB/len(ListRealizationsValuesLLGMMTrain))
    
    
    pl.figure()
    pl.plot(range(1,len(avr_RealizationsValuesLLGMMTrain)),avr_RealizationsValuesLLGMMTrain[1:len(avr_RealizationsValuesLLGMMTrain)],'-bs',markeredgecolor = 'k')
    pl.plot(range(1,len(avr_RealizationsValuesLLGMMTest)),avr_RealizationsValuesLLGMMTest[1:len(avr_RealizationsValuesLLGMMTest)],'-rs',markeredgecolor = 'k')
    pl.plot(avr_RealizationsValuesLLGMMTest.index(np.max(avr_RealizationsValuesLLGMMTest)),np.max(avr_RealizationsValuesLLGMMTest),'yp',markeredgecolor = 'k')
...
#    pl.plot(ValuesLLGMMTest.index(np.max(ValuesLLGMMTest)),np.max(ValuesLLGMMTest),'yp',markeredgecolor = 'k')
...
#    pl.yscale('log')
    ...
    pl.savefig('Drawings/'+'loglikelihookGMMTheOne.eps')
    pl.show()
    
    print "max log likelihood " + str(np.max(ValuesLLGMMTest)) + " at index " + str(ValuesLLGMMTest.index(np.max(ValuesLLGMMTest)))
...
#    #Drawing first probability gaussian
    SelectZ = 0    
    ZZg1 = computeSurfaceZ(XX,YY,[maximumLista_mu[SelectZ]],[maximumLista_CV[SelectZ]],[maximumv_pi[SelectZ]])
#    pl.figure()
...
#    #Drawing second probability gaussian
    SelectZ = 1    
    ZZg2 = computeSurfaceZ(XX,YY,[maximumLista_mu[SelectZ]],[maximumLista_CV[SelectZ]],[maximumv_pi[SelectZ]])
#    pl.figure()
...
#    #Drawing thrid probability gaussian
    SelectZ = 2    
    ZZg3 = computeSurfaceZ(XX,YY,[maximumLista_mu[SelectZ]],[maximumLista_CV[SelectZ]],[maximumv_pi[SelectZ]])
#    pl.figure()
...
#    while(True):
    while(iteration < NIterations):
        ...
        print stringName
        Lista_mu_T_1 = list(Lista_mu)
        resultUpdate = updateParameters_k_means(v_pi, XTrain,Lista_mu,Lista_CV,NClusters)
#        resultUpdate = updateParameters_K_MEANS(XTrain,Lista_mu,Lista_CV,NClusters)
        Lista_mu = resultUpdate[0]
        pathKmeans.append(Lista_mu)
        Lista_CV = []
        for CVm in resultUpdate[1]:
            Lista_CV.append(np.matrix(CVm))
...
#        ValuesLLKmeansTest.append(calculate_LogLikelihood(Lista_mu,Lista_CV,v_pi,XTest))
        ValuesLLKmeansTrain.append(calculate_distortionMeasure(Lista_mu,Lista_CV,resultUpdate[2],XTrain,NClusters))
        ValuesLLKmeansTest.append(calculate_distortionMeasure(Lista_mu,Lista_CV,resultUpdate[2],XTest,NClusters))
#        calculate_distortionMeasure(List_mu,List_CVMatrices,r_k,X,Features)
        print "Distortion Measure "+ str(ValuesLLKmeansTest[-1])
        ...
        print "norm(tempM)\n"+str(NormMagnitudeDisplacement)
        iteration = iteration + 1
        #Drawing here
#        ZZ = computeSurfaceZ(XX,YY,Lista_mu,Lista_CV,v_pi)
        ZZ_1 = computeSurfaceZ_ind_dist(XX,YY,Lista_mu[0],Lista_CV[0])
        ZZ_2 = computeSurfaceZ_ind_dist(XX,YY,Lista_mu[1],Lista_CV[1])
        ZZ_3 = computeSurfaceZ_ind_dist(XX,YY,Lista_mu[2],Lista_CV[2])
        pl.figure()
        
        pl.plot(Data_1[0],Data_1[1],'bo',markeredgecolor = 'k')
        pl.plot(Data_2[0],Data_2[1],'ro',markeredgecolor = 'k')
        pl.plot(Data_3[0],Data_3[1],'go',markeredgecolor = 'k')
        CS1 = pl.contour(XX, YY, ZZ_1, [0.001,0.002,0.003,0.004,0.005], cmap = mapaColores)
        CS2 = pl.contour(XX, YY, ZZ_2, [0.001,0.002,0.003,0.004,0.005], cmap = mapaColores)
        CS3 = pl.contour(XX, YY, ZZ_3, [0.001,0.002,0.003,0.004,0.005], cmap = mapaColores)
#        CS = pl.contour(XX, YY, ZZ)
        pl.clabel(CS1, inline=1, fontsize=10)
        pl.clabel(CS2, inline=1, fontsize=10)
        pl.clabel(CS3, inline=1, fontsize=10)
...
#    pl.savefig('Preliminary.png')
    ...
    pl.figure()
    pl.plot(range(1,len(ValuesLLGMMTrain)),ValuesLLGMMTrain[1:len(ValuesLLGMMTrain)],'-bs',markeredgecolor = 'k')
    pl.plot(range(1,len(ValuesLLKmeansTest)),[float(element) for element in ValuesLLKmeansTest[1:len(ValuesLLKmeansTest)]],'-rs',markeredgecolor = 'k')
#    pl.plot(range(1,len(ValuesLLGMMTrain)),[-elem for elem in ValuesLLGMMTrain[1:len(ValuesLLGMMTrain)]],'-bs',markeredgecolor = 'k')
#    pl.plot(range(1,len(ValuesLLGMMTest)),[-elem for elem in ValuesLLGMMTest[1:len(ValuesLLGMMTest)]],'-rs',markeredgecolor = 'k')
#    pl.yscale('log')
    pl.grid()
    pl.title('distortion measure K-means')
